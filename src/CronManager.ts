...
export class CronManager {
  ...
  public static registerController(classes: Function[]): void {
    ...
    jobs.forEach((job) => {
      job.crons.forEach((c) => {
        try {
          ...
          log.warn(`Could not mount cron '${c.name}':`, error.message);
        }
      });
    });
    ...
    this.cronJobs.forEach((cj) => {
      try {
        ...
        log.warn(`Could not start cron '${cj.cronMetadata.name}':`, error.message);
      }
    });
  }
  ...
  public static getCrons(namespace?: string): Map<string, cron.CronJob> {
    const result = new Map<string, cron.CronJob>();
